...
# Batch evaluation script for random transfer between all compressors
# For each source compressor, generates random vectors in target decoder space
...
TRANSFER_BATCH_SIZE="${TRANSFER_BATCH_SIZE:-256}"  # Batch size for memory transfer conversion
...
OUTPUT_BASE_DIR="${OUTPUT_BASE_DIR:-outputs/simple_compressor/transfer_compressor/random_transfer}"
...
echo "========================================"
echo "Random Transfer Batch Evaluation"
echo "========================================"
...
# Preflight: ensure transfer.py supports random converter
# if ! python - <<'PY' >/dev/null 2>&1
# import sys
# from pathlib import Path

# repo_root = Path(".").resolve()
# sys.path.append(str(repo_root / "src/soft_compress/converter"))
# from converter_factory import converter_classes

# if "random" not in converter_classes:
#     raise SystemExit(1)
# PY
# then
#     echo -e "${RED}Error: current runtime does not support converter_type=random${NC}"
#     echo -e "${YELLOW}Please sync these files to the runtime environment first:${NC}"
#     echo "  - src/soft_compress/converter/random_converter.py"
#     echo "  - src/soft_compress/converter/converter_factory.py"
#     echo "  - src/soft_compress/transfer_compressor/transfer.py"
#     exit 1
# fi
...
for src_compressor_dir in "${COMPRESSORS[@]}"; do
    ...
    for tgt_compressor_dir in "${COMPRESSORS[@]}"; do
        ...
        # Step 1: Transfer compression using random method
        echo "Step 1: Transferring compression (random method)..."
        transfer_results_file="${transfer_output_dir}/compression_results.json"

        mkdir -p "$transfer_output_dir"

        # Random baseline: ignore source memory content and directly sample random
        # vectors in the target decoder embedding space.
        CUDA_VISIBLE_DEVICES=${GPU_ID} python "${TRANSFER_SCRIPT}" \
            ...
            --converter_kwargs "{\"random_seed\": ${RANDOM_SEED}}" \
            --device cuda \
...
echo ""
echo "========================================"
echo -e "${BLUE}Random Transfer Batch Evaluation Summary${NC}"
echo "========================================"
